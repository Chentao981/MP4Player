...
- (void)addEventListenerWithType:(NSString *)type target:(id)target action:(SEL)action {
    [self addEventListenerWithType:type target:target action:action priority:1];
}
...
- (void)addEventListenerWithType:(NSString *)type target:(id)target action:(SEL)action priority:(NSUInteger)priority {
    ...
    for (NSInteger i = 0; i < listeners.count; i++) {
        ...
        if (tempListener.target == target && [actionStr isEqualToString:tempListener.action]) {
            return;
        }
    }
...
- (void)removeEventListenerWithType:(NSString *)type target:(id)target action:(SEL)action {
    ...
    if (eventPriority) {
        ...
        for (NSInteger keyIndex = keys.count - 1; keyIndex >= 0; keyIndex--) {
            ...
            for (NSInteger listenerIndex = listeners.count - 1; listenerIndex >= 0; listenerIndex--) {
                CEventListenerTarget *listener = [listeners objectAtIndex:listenerIndex];
                if (listener.target == target && [listener.action isEqualToString:actionStr]) {
                    [listeners removeObject:listener];
                }
            }
...
- (void)removeAllEventListener {
    NSMutableDictionary *eventListeners = [self eventListeners];

    for (NSString *eventType in eventListeners) {
        CEventPriority *eventPriority = [eventListeners objectForKey:eventType];
        eventPriority.typeEventListeners = [[NSMutableDictionary alloc] init];
        eventPriority.priorityArray = [[NSMutableArray alloc] init];
        for (NSNumber *priorityNumber in eventPriority.priorityArray) {
            NSString *priorityKey = [NSString stringWithFormat:@"%@", priorityNumber];
            NSMutableArray *listeners = [eventPriority.typeEventListeners objectForKey:priorityKey];
            NSInteger listenersIndex = listeners.count - 1;
            for (NSInteger index = listenersIndex; index >= 0; index--) {
...
- (void)executeListener:(CEventListenerTarget *)listener andEvent:(CEvent *)event {
    id target = listener.target;
    if (target) {
